...
export default function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const SERVICE_ID = "service_djhtqdf";
    const TEMPLATE_ID = "template_ztj1gpq";
    const PUBLIC_KEY = "BGiJA_ZIMPD4hyJEj";
  ...
  const handleMapOpen = () => {
    window.open(`https://www.google.com/maps/search/?api=1&query=${YOUR_MAPS_QUERY}`, "_blank");
  };
            ...
            <div className="quick-contact-grid">
              ...
              <button className="quick-btn call-btn" onClick={handleCall}>
                ...
                <span className="quick-btn-text">
                  ...
                  <span className="quick-btn-sub">{YOUR_PHONE_NUMBER}</span>
                </span>
              </button>

              <button className="quick-btn map-btn" onClick={handleMapOpen}>
                <span className="quick-btn-icon">
                  <svg viewBox="0 0 24 24" fill="currentColor" width="22" height="22">
                ...
                <span className="quick-btn-text">
                  <span className="quick-btn-label">View on Google Maps</span>
                  <span className="quick-btn-sub">Open our location pin</span>
                </span>
              </button>
            </div>
          </div>
          ...
          <div className="contact-container">
            <div className="contact-grid">
              ...
              <div className="contact-info-panel">
                ...
                <div className="info-section">
                  <h2>Find Us</h2>
                  <div className="map-preview" onClick={handleMapOpen} title="Open in Google Maps">
                    <iframe
                      title="Office Location"
                      src={`https://maps.google.com/maps?q=${YOUR_MAPS_QUERY}&output=embed`}
                      width="100%"
                      height="200"
                      style={{ border: 0, borderRadius: "6px", pointerEvents: "none" }}
...
.quick-contact-grid { max-width: 1100px; margin: 0 auto; display: grid; grid-template-columns: repeat(3, 1fr); gap: 1rem; }
...
/* Map */
.map-btn { border-color: #4285F4; }
.map-btn:hover { background: #f0f4ff; border-color: #4285F4; }
.map-btn .quick-btn-icon { background: #4285F4; color: #fff; }
.map-btn .quick-btn-label { color: #4285F4; }
...
/* ✅ Map Preview */
.map-preview { position: relative; cursor: pointer; border-radius: 6px; overflow: hidden; border: 1.5px solid #e8dcc8; transition: border-color 0.2s; }
.map-preview:hover { border-color: #4285F4; }
.map-overlay { position: absolute; bottom: 0; left: 0; right: 0; background: rgba(13, 18, 8, 0.75); color: #fff; font-size: 0.82rem; font-weight: 600; display: flex; align-items: center; justify-content: center; padding: 0.6rem; gap: 0.4rem; transition: background 0.2s; }
.map-preview:hover .map-overlay { background: rgba(66, 133, 244, 0.88); }
.map-overlay span { display: flex; align-items: center; gap: 6px; }
